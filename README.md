...
로컬에서 자주 사용하는 명령과 CI에 기대되는 동작을 정리합니다.
...
- Vitest는 이 저장소에서 `jsdom` 환경으로 구성되어 있으며, `test/setupTests.js`를 통해 `@testing-library/jest-dom` 매처를 Vitest의 `expect`에 확장합니다.
- 새로운 테스트를 추가할 때는 `test/*.test.jsx` 또는 `test/*.spec.jsx` 패턴을 사용하세요.
